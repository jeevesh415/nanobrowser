...
const getSessionMessagesStorage = (
  sessionId: string,
  config?: { liveUpdate?: boolean },
) => {
...
export function createChatHistoryStorage(): ChatHistoryStorage {
  return {
    ...
    clearAllSessions: async (): Promise<void> => {
      ...
      // Clear all session messages concurrently
      // We disable liveUpdate during this bulk operation to prevent registering
      // unnecessary storage listeners for every session being deleted
    ...
    deleteMessage: async (sessionId: string, messageId: string): Promise<void> => {
      ...
      await Promise.all([
        ...
        messagesStorage.set(prevMessages => prevMessages.filter(msg => msg.id !== messageId)),

        // Update the session's metadata (updatedAt timestamp and messageCount)
        chatSessionsMetaStorage.set(prevSessions => {
          return prevSessions.map(session => {
            if (session.id === sessionId) {
              return {
                ...session,
