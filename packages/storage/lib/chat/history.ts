...
export function createChatHistoryStorage(): ChatHistoryStorage {
  return {
    ...
    clearAllSessions: async (): Promise<void> => {
      ...
      const clearMessagesPromises = sessionsMeta.map(sessionMeta => {
        // Use liveUpdate: false to prevent listener memory leaks during bulk operations
        const messagesStorage = getSessionMessagesStorage(sessionMeta.id, { liveUpdate: false });
        return messagesStorage.set([]);
      });

      await Promise.all([...clearMessagesPromises, chatSessionsMetaStorage.set([])]);
    },
